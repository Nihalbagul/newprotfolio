...
const MAX_WAIT_MS = 4000; // Safety cap — never block for more than 4s

const Loader = () => {
  const [isLoaded, setIsLoaded] = useState(false);

  useEffect(() => {
    const dismiss = () => setIsLoaded(true);

    // Primary trigger: actual window load event (fires when all resources ready)
    if (document.readyState === 'complete') {
      // Already loaded before this component mounted (HMR / fast connections)
      dismiss();
      return;
    }
    ...
    // Safety cap: never show longer than MAX_WAIT_MS regardless of load state
    const timeout = setTimeout(dismiss, MAX_WAIT_MS);
  ...
  return (
    <AnimatePresence>
      {!isLoaded && (
        <motion.div
          className="circuit-loader-wrapper"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
